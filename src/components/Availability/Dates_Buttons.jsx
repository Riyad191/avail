...
const ButtonsAndDates = ({item, dataArrowsAndColors, modalStates, lastFiveDays, setOpenModal}) => {
  ...
  let newCreateDate = recentFiveDays.filter(val=>![...new Set(item.date_and_percentage.map(z=>z.map(a=>a.create_date.slice(0,10)).filter(m=>m!=="no data av")).map(t=>[...new Set(t)]).flat())].includes(val));
  for(let i=0;i<recentFiveDays.length;i++){item.date_and_percentage.length<recentFiveDays.length&&item.date_and_percentage.push(new Array(288).fill({create_date:newCreateDate[i]+"T00:00:00.000000".slice(0,10), avail_percent:0}))}
  item.date_and_percentage.sort((x,y) =>{
    let valueA = [...new Set(x.map(a => a.create_date.slice(0,10)))].filter(a => a !== "no data av").flat()
    let valueB = [...new Set(y.map(a => a.create_date.slice(0,10)))].filter(a => a !== "no data av").flat()
                  ...
                  <Box onClick={() => setOpenModal(true)} key={idx}>
                    <Tooltip title={`click`} placement="top" followCursor>
                      <Box sx={{margin: "0 20px"}} >
                        ...
                        <Box sx={{width: "50px"}} >{`${boxDate.map(a => a.slice(5))}`}</Box>
                      </Box>
                    </Tooltip>
                  </Box>
                );
