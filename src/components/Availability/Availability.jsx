...
function Availability() {
  ...
  const availabilityDateFrom = useSelector(
    (state) => state.pillarNameReducer.availabilityDateFrom
  );
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .then((res) => {
        ...
        res.forEach((dateData) => {
          dateData.forEach((appData) => {
            const newArr = new Array(288).fill({
              create_date: "no data available",
            });
  ...
  useEffect(() => {
    ...
    fiveMinsDataWithDates.map((a) => {
      const tot = a.date_and_percentage[0]
        .map((a) => a.avail_percent)
        .filter((b) => b !== undefined);
      let total =
        tot.length > 0
          ? tot.reduce((acc, cur) => acc + cur, 0) / tot.length
          : tot.reduce((acc, cur) => acc + cur, 0);
      totalArr.push(total);
      barsData = a.date_and_percentage[0]
        .slice(0, 24)
        .map((a) => a.create_date);
    });
    const todaysAvailibilty =
      totalArr.reduce((acc, cur) => acc + cur, 0) /
      fiveMinsDataWithDates.length;
  ...
  const searchFn = (rows) =>
    rows
      .filter((row) => row.app_name.indexOf(searchAppName) > -1)
