...
const ButtonsComponent = () => {
  ...
  React.useEffect(() => {
    navigate("/availability/Transportation");
  }, []);
  ...
  return (
    <Box
      sx={{ height: "15%", color: "#142A7C", width: "100%", padding: "10px" }}
    >
        ...
        {pillars.map((pillar, idx) => {
          return (
            <Link key={idx} to={`/availability/${pillar.name}`}>
              <StyledButton
                onClick={() => showDetails(pillar.name, idx)}
                style={{
                  background: pillar.name == pillarName && "#fff",
                  color: pillar.name == pillarName && "#001051",
                }}
                key={idx}
                variant="outlined"
              >
